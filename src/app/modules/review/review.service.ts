...
const createReview = async (payload: IReview, user: JwtPayload) => {
  ...
  try {
    ...
    // Aggregate reviews for the product (with session and ObjectId match)
    const reviews = await Review.aggregate([
      {
        $match: {
          product: new mongoose.Types.ObjectId(review[0].product.toString()),
        },
      },
      {
        $group: {
    ...
    ]).session(session); // IMPORTANT: attach session
